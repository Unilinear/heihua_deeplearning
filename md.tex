...
\title{Deep Learning Notation Cheat Sheet}
\author{}
\date{}
...
\section*{Common Deep Learning Symbols}

\begin{longtable}{>{\centering\arraybackslash}p{2.2cm} p{3.5cm} p{5cm} p{3.5cm}}
\toprule
\textbf{Symbol} & \textbf{Often Means} & \textbf{Plain English Meaning} & \textbf{Common Alternatives} \\
\midrule
\endfirsthead
...
\[
\text{Find the best parameters } \theta^*
\text{ that minimize the loss } \mathcal{L}(\theta).
\]
...
\subsection*{Encoder and Classifier Example}
...
f_\phi
=
\text{encoder with parameters } \phi,
\]

\[
g_\theta
=
\text{classifier or decoder with parameters } \theta.
\]

So $\phi$ and $\theta$ are both parameters, but they belong to different parts of the model.

\section*{How to Decode Symbols in a Paper}
...
\toprule
\textbf{Question} & \textbf{Common Symbols} \\
\midrule
...
\[
\boxed{
\theta,\ w,\ W,\ \phi,\ \psi,\ \omega
\text{ often all mean learnable parameters.}
}
\]

Authors use different letters mainly to separate different parts of the model.
...
p_\theta
=
\text{decoder or generative model},
\]

\[
q_\phi
=
\text{encoder or approximate inference model}.
\]
